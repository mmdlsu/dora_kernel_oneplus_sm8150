...
u8 halrf_match_iqk_version(void *dm_void)
{
    ...
    /*
     * 说明：原实现使用 odm_move_memory(=memcpy) 以固定长度(10)从字符串常量
     * HALRF_IQK_VER 拷贝到临时缓冲区，触发了内核 FORTIFY 的 __read_overflow2
     * 检测（从较小源对象读取超过其大小的字节）。
     * 这里改为使用 snprintf 安全拷贝，避免对源大小的越界读取，从而修复链接期
     * undefined symbol: __read_overflow2 的问题。
     */
    snprintf(temp, sizeof(temp), "%s", HALRF_IQK_VER);
